...
- **调整遍历命令适用范围**：遍历命令仅支持普通二叉树（BinaryTree），不支持在 BST/AVL/Huffman 上执行。
...
菜单栏新增 **“历史”**，用于查看应用会话中的 DSL 操作记录：
- **查看线性历史**：显示线性结构的 DSL 操作记录
- **查看树形历史**：显示树形结构的 DSL 操作记录
- **查看全部历史**：合并显示线性与树形的记录，并在每行前显示上下文标签 `[linear]`、`[tree]` 或 `[global]`

显示与说明：
- **时间戳**：仅显示时间，格式 `HH:MM:SS`
- **边界规则**：历史按“边界”截取展示（全局 `clear` 或当前上下文的 `create/build` 事件之后的记录）。合并视图会按时间顺序合并两侧记录，并包含一个较新的全局 `clear` 作为边界行（若存在）
- **彩色高亮**：
  - 时间戳为灰色
  - 上下文标签颜色区分：`[linear]` 蓝色、`[tree]` 绿色、`[global]` 橙色
  - 行首动词加粗紫色（如 `create`、`insert`、`delete`、`search`、`traverse`、`build`、`encode`、`decode`、`clear`）
  - 数字为绿色；双引号中的文本为橙色
- **字体**：使用等宽字体，字号为 20px，便于对齐与阅读
- **只读与复制**：历史对话框只读，但支持复制文本
- **存储与清空**：历史为内存记录，应用重启后清空；暂未提供“清空历史/导出历史”菜单项
...
5. **遍历树**：
   - 当前版本不支持在二叉搜索树上执行遍历命令，请使用 BinaryTree 进行遍历。
...
5. **遍历树**：
   - 当前版本不支持在 AVL 树上执行遍历命令，请使用 BinaryTree 进行遍历。
...
本模拟器支持使用领域特定语言（DSL）来操作数据结构。您可以在主界面的命令输入框中输入DSL命令。界面按钮触发的操作也会自动映射为 DSL 并记录在历史中（便于回溯或复制）。
